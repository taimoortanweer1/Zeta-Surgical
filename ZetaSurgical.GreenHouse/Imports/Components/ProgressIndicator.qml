        ...
        states: [
            State {
                ...
                PropertyChanges {
                    ...
                    height: background.height * Math.min(1.0 ,((root.value - root.min)/Math.max(1, root.max - root.min)))
                }
                AnchorChanges {
                    target: indicator

                    anchors.left: indicator.parent.left
                    anchors.right: indicator.parent.right
                    anchors.top: undefined
                    anchors.bottom: indicator.parent.bottom
                }
            }
        ]
    }
