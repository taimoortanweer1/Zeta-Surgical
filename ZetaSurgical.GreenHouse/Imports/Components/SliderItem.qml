...
Item {
    ...
    Image {
        ...
        property int minX: root.knobInset ? root.fillMargin : -width * 0.5
        property int minY: root.knobInset ? root.fillMargin : -height * 0.5
        ...
        x: {
            if (!root.__vertical && !knobMa.drag.active)
                return ((knobMa.drag.maximumX - knobMa.drag.minimumX)
                        * ((internal.value - root.min) / Math.max(1, root.max - root.min))) + minX
            return minX;
        }
        y: {
            if (root.__vertical && !knobMa.drag.active)
                return ((knobMa.drag.maximumY - knobMa.drag.minimumY)
                        * ((internal.value - root.min) / Math.max(1, root.max - root.min))) + minY
            return minY;
        }
        z: 1
        ...
        MouseArea {
            ...
            drag.maximumX: root.width - knob.width - knob.minX
            drag.minimumY: knob.minY
            drag.maximumY: root.height - knob.height - knob.minY
        ...
        onXChanged: {
            if (!root.__vertical && knobMa.drag.active) {
                internal.value = internal.valueForPosition(x - knobMa.drag.minimumX,
                                                           knobMa.drag.maximumX - knobMa.drag.minimumX)
        ...
        onYChanged: {
            if (root.__vertical && knobMa.drag.active) {
                internal.value = internal.valueForPosition(y - knobMa.drag.minimumY,
                                                           knobMa.drag.maximumY - knobMa.drag.minimumY)
                if (root.dynamicValueReporting)
                    root.valueChangeRequested(internal.value)
            }
        }
